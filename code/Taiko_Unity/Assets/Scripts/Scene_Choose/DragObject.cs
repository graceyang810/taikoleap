...
public class DragObject : IgnoreTimeScale
{
	...
	void Start () {
		...
		for(int m=0;m<count;m++)
		{
			...
			TweenColor.Begin(songs[m], 0.2f, i ? new Color(0.7f,0.7f,0.7f) : new Color(0.7f,0.7f,0.7f));
		...
		if (enabled && NGUITools.GetActive(gameObject) && target != null)
		{
			...
			if (mPlane.Raycast(ray, out dist))
			{
				...
				//	target.position +=new Vector3(0,offset.y,0);	
					...
					//----------Test----------------	
						//	TweenPosition.Begin(songs[p],0.4f,songs[p].transform.position+new Vector3(0,offset.y,0));
					//		int q=order[k-1];
					//		if(k-1>0)
					//	//	TweenScale.Begin(songs[p],0.2f,new Vector3(offset.y*10+0.003f,offset.y*10+0.003f,0));
					//		TweenTransform.Begin(songs[p],1f,songs[q].transform);
					//	TweenPosition.Begin()
							//----------Test----------------
						}
...
//		Debug.Log(target.position.y);
	...
	void formatAlpha()//fit in rotation
	{
		...
		for(int m=0;m<count;m++)
		{
			...
			TweenColor.Begin(songs[t], 0.2f, i ? new Color(0.7f,0.7f,0.7f) : new Color(0.7f,0.7f,0.7f));//all songs set gray			
		}
		int q=order[1];	
		Color mColor;
	...
	//	targetPic.GetComponent<UISprite>().spriteName="";
		...
		UISprite spriteLevel = targetLevel.GetComponent<UISprite>();
		
		if(songs[p].name=="ChristSongs")
		{
			sprite.spriteName = "songDescription3";
			spriteLevel.spriteName="hard4";
		}
	...
	void playSound()
	{
		NGUITools.PlaySound(CbuttonOut, volume, pitch);
		int q=order[1];
		audioSouces[forePoint].Stop();
		audioSouces[q].Play();
	}
	/// <summary>
